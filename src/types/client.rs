...
impl Client {
    pub async fn connect(tg_client: TGClient, is_completed: bool) -> Result<Self, ProcessError> {
        if is_completed {
            let user = tg_client
                .get_me()
                .await
    ...
    pub async fn connect_to_saved_sessions(
        secret_data: APIKeys,
    ) -> Result<BTreeMap<String, Client>, ProcessError> {
        ...
        while let Some(Ok(session_file)) = session_files.next() {
            ...
            let tg_client = TGClient::connect(Config {
                ...
                api_hash: secret_data.api_hash.clone(),
                params: Default::default(),
            })
            .await
            .map_err(|_| ProcessError::ClientIsNotConnected)?;

            let client = Self::connect(tg_client, true).await?;
    ...
    pub async fn send_login_code(
        phone_number: &str,
        secret_data: &APIKeys,
    ) -> Result<(LoginToken, Client), ProcessError> {
        let tg_client = TGClient::connect(Config {
            session: Session::new(),
            api_id: secret_data.api_id,
            api_hash: secret_data.api_hash.clone(),
        ...
        Ok((login_token, Client::connect(tg_client, false).await?))
    }
    ...
    pub async fn sign_in(
        ...
        let client = Client::connect(self.tg_client, true).await?;
    ...
    pub async fn get_uploaded_files(self) -> Result<(String, Vec<File>), ProcessError> {
        ...
        Ok((self.get_username()?, files))
    }

    pub async fn download_files(self, message_ids: Vec<i32>) -> Result<(), ProcessError> {
        let mut messages = self.get_messages_by_id(&message_ids).await?;

        fn get_home_directory() -> Result<String, ProcessError> {
            match home_dir() {
            ...
            if let Some(media) = message.clone().media() {
                match media {
                    Media::Document(document) => {
                        download_file(message, document.name()).await?
                    }
    ...
    pub async fn delete_files(self, message_ids: Vec<i32>) -> Result<(), ProcessError> {
        let mut messages = self.get_messages_by_id(&message_ids).await?;
